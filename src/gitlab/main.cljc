(ns gitlab.main
  "Kotodama WASM entrypoint for the Gitlab clean-room actor (L4) — Clojure port.
  ...
  facts (`gitlab.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gitlab")
(def tier "L4")
...
(def entity-specs
  [{:entity "Project"      :plural "projects"      :id-prefix "gitlab_pro"
    :fields [:name :path :description :visibility :defaultBranch :archived] :required [:name :path]
    :coerce {}                                   :refs {}}
   {:entity "Issue"        :plural "issues"        :id-prefix "gitlab_iss"
    :fields [:projectId :iid :title :description :state :issueType] :required [:iid :title]
    :coerce {:projectId :int :iid :int}         :refs {:projectId "Project"}}
   {:entity "MergeRequest" :plural "mergerequests" :id-prefix "gitlab_mer"
    :fields [:projectId :iid :title :state :sourceBranch :targetBranch :sha] :required [:iid :title]
    :coerce {:projectId :int :iid :int}         :refs {:projectId "Project"}}
   {:entity "Pipeline"     :plural "pipelines"     :id-prefix "gitlab_pip"
    :fields [:projectId :status :ref :sha :duration] :required [:status :ref]
    :coerce {:projectId :int :duration :int}    :refs {:projectId "Project"}}
   {:entity "Job"          :plural "jobs"          :id-prefix "gitlab_job"
    :fields [:name :status :stage :ref]             :required [:name :status]
    :coerce {}                                   :refs {}}
   {:entity "Commit"       :plural "commits"       :id-prefix "gitlab_com"
    :fields [:shortId :title :message :authorName :authorEmail] :required [:title :message]
    :coerce {}                                   :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"gitlab.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gitlab-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gitlab-compat/ready)
